    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="section-container flex items-center justify-between h-16 md:h-20">
        <a href="#hero" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }); }}
          ...
          <span className="text-[var(--accent-olive)]">BSK</span><span className="text-[var(--text-muted)]">.</span>
        </a>
            ...
            <li key={item.href}>
              <button onClick={() => handleClick(item.href)}
                className={`font-mono text-xs tracking-[0.12em] uppercase transition-colors duration-300 ${
                  activeSection === item.href ? "text-[var(--accent-olive)]" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"
                }`}>
                {item.label}
              </button>
            </li>
              ...
              <li key={item.href}>
                <button onClick={() => handleClick(item.href)}
                  className={`font-mono text-sm tracking-[0.12em] uppercase transition-colors w-full text-left py-2 ${
                    activeSection === item.href ? "text-[var(--accent-olive)]" : "text-[var(--text-muted)]"
                  }`}>
                  {item.label}
                </button>
              </li>
