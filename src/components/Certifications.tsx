      ...
      <div className="section-container">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {certifications.map((cert, i) => (
            ...
            >
              <div className="card-icon-wrapper w-12 h-12 rounded-lg bg-[rgba(139,92,246,0.1)] flex items-center justify-center mx-auto">
                <Award size={20} className="text-[var(--accent-purple)]" />
              </div>
              ...
              {"details" in cert && cert.details && (
                <p className="text-xs text-[var(--accent-purple)] font-medium">{cert.details}</p>
              )}
              {"verifyUrl" in cert && cert.verifyUrl && (
                <a href={cert.verifyUrl} target="_blank" rel="noopener noreferrer"
                  className="hover-icon-slide inline-flex items-center gap-1.5 text-xs text-[var(--accent-purple)] hover:underline">
