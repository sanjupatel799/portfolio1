  ...
  return (
    <div style={{ animation: `fadeInUp 0.5s ease ${index * 0.08 + 0.2}s forwards`, opacity: 0 }}>
      ...
      <div className="w-full h-1.5 bg-[rgba(255,255,255,0.05)] rounded-full overflow-hidden">
        <div
          className="h-full rounded-full bg-gradient-to-r from-[var(--accent-olive)] to-[var(--accent-lime)]"
...
export default function Skills() {
  return (
    <section id="skills" className="relative py-44 md:py-52 overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[700px] bg-[var(--accent-olive)] opacity-[0.015] blur-[140px] rounded-full" />
      ...
      <div className="section-container">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {skillCategories.map((cat, i) => (
            <div
              key={cat.title}
              className="glass rounded-xl px-12 py-8 flex flex-col gap-5 transition-all duration-300 hover:border-[var(--accent-olive)] hover:glow-olive hover:-translate-y-1"
              style={{ animation: `fadeInUp 0.5s ease ${i * 0.08}s forwards`, opacity: 0 }}
            >
              <h3 className="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-[var(--accent-olive)] font-medium pb-3 border-b border-[var(--border-color)] text-center">
                // {cat.title}
              </h3>
        ...
        <div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {techTags.map((tag) => (
              <span key={tag} className="hover-tag-pop px-3.5 py-1.5 glass text-xs font-mono text-[var(--text-secondary)] rounded-md hover:border-[var(--accent-olive)] hover:text-[var(--accent-olive)] hover:glow-olive transition-all duration-200">
                {tag}
              </span>
            ))}
          </div>
